...
# Phase 4: Build Process (optional)
if (-not $SkipBuild) {
    Write-Host "`n4. Building reports..." -ForegroundColor Blue
    ...
    Write-Host "`n4. Skipping build process" -ForegroundColor Yellow
}
...
# Update PHP files with cache busting parameters
$phpFiles = @(
    "reports/index.php",
    "dashboard.php", 
    "index.php",
...
# Update JavaScript files with cache busting
$jsFiles = @(
    "reports/dist/reports.bundle.js",
    "reports/dist/reports-main.js"
)
...
# Create cache-busting manifest file
$manifestContent = @{
...
try {
    Set-Content "cache-bust-manifest.json" $manifestContent
...
Write-Host "   Cache busting completed successfully" -ForegroundColor Green

# Phase 6: Testing Preparation
Write-Host "`n6. Preparing for testing..." -ForegroundColor Magenta
...
Write-Host "`nCache Busting Commands:" -ForegroundColor Cyan
Write-Host "   View cache manifest: Get-Content cache-bust-manifest.json" -ForegroundColor White
Write-Host "   Force cache bust: Remove-Item cache-bust-manifest.json; .\scripts\start-local-testing-simple.ps1" -ForegroundColor White
