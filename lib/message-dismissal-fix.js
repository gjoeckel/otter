/**
 * Message Dismissal Utility - Production Fix
 * Enhanced version that handles production server path differences
 */
...
class MessageDismissal {
    ...
    focusErrorInput() {
        // Enhanced page detection that handles production paths
        ...
        let inputSelector = errorInputMap[currentPage];
        
        // If no exact match, try to find by filename
        if (!inputSelector) {
            const filename = this.extractFilename(currentPage);
            inputSelector = errorInputMap[filename];
        }
        ...
        if (inputSelector) {
            ...
            if (inputElement) {
                ...
                if (currentPage !== 'settings/index.php' && filename !== 'settings/index.php') {
    ...
    addErrorDismissalListeners() {
        ...
        // Try exact match first
        let inputSelector = errorInputMap[currentPage];
        let config = dismissalConfig[currentPage] || { strategy: 'non-empty', clearInput: true };
        
        // If no exact match, try to find by filename
        if (!inputSelector) {
            const filename = this.extractFilename(currentPage);
            inputSelector = errorInputMap[filename];
            config = dismissalConfig[filename] || { strategy: 'non-empty', clearInput: true };
        }
        ...
        if (inputSelector) {
            ...
            if (inputElement) {
                ...
                console.log('Input element not found for dismissal selector:', inputSelector);
            }
        } else {
            console.log('No input selector found for dismissal on page:', currentPage);
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Initializing enhanced MessageDismissal...');
