                        ...
                        if (expandedBefore) {
                            ...
                            if (filterWidget) {
                                setTimeout(() => {
                                    ...
                                    const twoRem = 2 * rem;
                                    const baseOffset = tableId === 'systemwide-data' ? twoRem : 30;

                                    // Prefer the inner fieldset as the scroll target if present
                                    const targetEl = filterWidget.querySelector('fieldset') || filterWidget;
                                    const legendEl = targetEl.querySelector('legend');
                                    const legendSpacingPx = 0.5 * rem;
                                    const legendAdjustment = legendEl ? (legendEl.offsetHeight + legendSpacingPx) : 0;

                                    const targetRectTop = targetEl.getBoundingClientRect().top + (window.pageYOffset || document.documentElement.scrollTop);
                                    const targetPosition = targetRectTop - headerHeight - baseOffset - legendAdjustment;
                                    console.log('[TFI] scrollTo:', { tableId, headerHeight, baseOffset, legendAdjustment, targetPosition });
